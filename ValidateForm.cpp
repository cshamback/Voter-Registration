...
void VoterForm::fillOutForm() {
  ...
  */
  ...
  cout << "Enter your Social Security number as a single number with no spaces or dashes
    (ex: 123456789) : " << endl; // Gets the Social Security number (SSN) of the user
  cin >> formRegistrationInfo[4];
  while (!validSSN) { // Ensures that the SSN entered is valid
    while (i < formRegistrationInfo[4].length()){
      if(formRegistrationInfo[4].find('-') != NULL ||
          formRegistrationInfo[4].find(' ') != NULL){
        cout << "Invalid SSN." << endl;
        cout << "Enter your Social Security number as a single number with no spaces or dashes
          (ex: 123456789): " << endl; // Gets the SSN of the user
      }
      else {
        validSSN = true;
      }
